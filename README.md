...
> **所要時間**: 1〜2 時間  
> **前提条件**: Microsoft Sentinel ワークスペース、SigninLogs・AzureActivity の診断設定が有効、**Windows VM に Log Analytics エージェント (MMA) または Azure Monitor エージェント (AMA) + 「Windows セキュリティ イベント」データコネクタが有効**
...
| 0:25〜0:40 | [Lab 0: リモートデスクトップ (RDP) 接続検知（特定条件）](#lab-0-リモートデスクトップ-rdp-接続検知特定条件パターン) |
| 0:40〜1:00 | [Lab 1: Azure VM 再起動検知（特定条件）](#lab-1-azure-vm-再起動検知特定条件パターン) |
| 1:00〜1:20 | [Lab 2: EntraID サインイン失敗閾値アラート（閾値）](#lab-2-entraid-サインイン失敗閾値アラート閾値パターン) |
| 1:00〜1:20 | [Lab 2: EntraID サインイン失敗閾値アラート（閾値）](#lab-2-entraid-サインイン失敗閾値アラート閾値パターン) |
| 1:20〜1:30 | [Lab 2-2: MFA チャレンジ失敗 閾値アラート（閾値）](#lab-2-2-mfa-チャレンジ失敗-閾値アラート閾値パターン) |
| 1:30〜1:50 | [Lab 3: 異常なサインインボリューム検知（アノマリー）](#lab-3-異常なサインインボリューム検知アノマリーパターン) |
| 1:40〜1:50 | [Lab 4: Azure RBAC 特権ロール割り当て検知（特定条件）](#lab-4-azure-rbac-特権ロール割り当て検知特定条件パターン) |
| 1:50〜2:00 | [Lab 5: 大量リソース削除検知（閾値）](#lab-5-大量リソース削除検知閾値パターン) |
| 2:00〜2:10 | [応用 Lab: 脅威アクター（パスワードスプレー）検知](#応用-lab-脅威アクター--パスワードスプレー攻撃midnight-blizzard-スタイル) |
| 2:10〜2:15 | [アラートグループ機能の設定](#3-アラートグループ機能の設定) |
...
| 項目 | 内容 |
|------|------|
| ルールの種類 | スケジュール済み / NRT (ほぼリアルタイム) / Microsoft Security / Anomaly |
...
| ルール種類 | 概要 | KQL 記述 | 実行間隔 | 主な用途 |
|-----------|------|---------|---------|---------|
| **スケジュール済み** | 任意の KQL クエリを定期実行し、結果をアラートに変換する最も汎用的なルール | ユーザーが記述 | 5 分〜1 日 | 閾値検知・特定条件検知・アノマリー検知（本ワークショップのメイン） |
| **NRT**（ほぼリアルタイム） | スケジュール済みと同じ KQL を使用するが、約 1 分間隔で実行される高頻度版 | ユーザーが記述 | 約 1 分 | 緊急性の高いアラート（特権操作・不正ログイン等）をリアルタイムに近い形で検知 |
| **Microsoft Security** | Microsoft Defender / Entra ID Protection 等の製品アラートを自動的に Sentinel インシデントに変換する | 不要（製品側が生成） | リアルタイム | Microsoft 製品のアラートを Sentinel で一元管理する際の連携ルール |
| **Anomaly**（UEBA） | ユーザー・エンティティの行動分析（UEBA）に基づき、機械学習で異常を検出するルール。カスタマイズ可能なカスタマイズ可能なフライトモードを持つ | 不要（ML モデルが自動生成） | 日次 | ユーザー行動の統計的異常（例: 通常と異なる時間帯のアクセス、新しい国からのサインイン） |
...
## Lab 0: リモートデスクトップ (RDP) 接続検知（特定条件パターン）

### シナリオ概要

Windows マシンへの**リモートデスクトップ (RDP) 接続**が発生した際にアラートを生成します。管理者が把握していない端末への不審なリモートアクセスを早期に発見することが目的です。

- **データソース**: `SecurityEvent`（Windows セキュリティイベント）
- **イベント ID**: `4624`（ログオン成功） ／ `LogonType = 10`（RemoteInteractive = RDP）
- **パターン**: 特定のイベント条件に合致したかどうかを判定する「特定条件パターン」
- **エンティティ**: ログオンアカウント・接続元 IP アドレス・接続先ホスト

> **前提**: Windows VM に Log Analytics エージェント (MMA) または Azure Monitor エージェント (AMA) がインストールされ、Microsoft Sentinel の「Windows セキュリティ イベント」データコネクタが有効になっていること。

### LogonType の種類（参考）

| LogonType | 種別名 | 説明 |
|-----------|-------|------|
| 2 | Interactive | コンソール（物理）ログオン |
| 3 | Network | ネットワーク共有などのネットワーク認証 |
| **10** | **RemoteInteractive** | **RDP によるリモートデスクトップ接続** |
| 7 | Unlock | ロック解除 |
...
| where AccountType == "User"                  // コンピューターアカウントを除外
| project
    TimeGenerated,
    Computer,                                  // 接続先ホスト名
    TargetAccount,                             // ログオンしたアカウント (DOMAIN\User 形式)
    TargetUserName,                            // ユーザー名のみ
    IpAddress,                                 // 接続元 IP アドレス
    LogonTypeName,
    EventID
...
| 行 | 説明 |
|----|------|
| `EventID == 4624` | Windows ログオン成功イベント。失敗（4625）は検知しないことで「接続完了した端末」に絞り込む |
| `LogonType == 10` | RDP 専用のログオンタイプ。値 `3`（ネットワーク）や `2`（コンソール）を含めると対象が広がりすぎる |
| `AccountType == "User"` | `$` で終わるコンピューターアカウントのログオンを除外し、人が操作した接続のみに絞り込む |
...
|------|--------|
| 名前 | `RDP 接続検知` |
| 説明 | Windows マシンへのリモートデスクトップ (RDP) 接続成功を検知します。 |
| 重大度 | 情報 (Informational) ※環境に応じて「低」や「中」に引き上げ可能 |
| 戦術と手法 | Lateral Movement > T1021.001 Remote Services: Remote Desktop Protocol |
| 状態 | 有効 |
...
2. 「**クエリのスケジュール設定**」
   - 実行間隔: **5 分ごと**（NRT ルールに変更することでほぼリアルタイム検知も可能）
   - データの参照: **過去 1 日**
3. 「**クエリ結果の警告しきい値**」
   - 次の場合にアラートを生成: **より大きい** → `0`

> **発展課題**: 業務時間外（例: 22:00〜06:00）の RDP 接続のみに絞り込むには、`where hourofday(TimeGenerated) !between (6 .. 21)` を追加してみましょう。
...
| エンティティ | 種類 | 識別子 | 列名 |
|-------------|------|--------|------|
| アカウント | Account | FullName | TargetAccount |
| ホスト | Host | HostName | Computer |
| IP アドレス | IP | Address | IpAddress |
...
- インシデントの自動作成: **有効**
- アラートのグループ化: **有効** → ホスト (Computer) エンティティで一致（同一端末への複数 RDP 接続をまとめる）
...
- **データソース**: `SigninLogs`
- **パターン**: KQL の [`series_decompose_anomalies`](https://learn.microsoft.com/ja-jp/azure/data-explorer/kusto/query/series-decompose-anomalies-function) を用いた「アノマリーパターン」
- **エンティティ**: ユーザーアカウント
...
| [`series_decompose_anomalies`](https://learn.microsoft.com/ja-jp/azure/data-explorer/kusto/query/series-decompose-anomalies-function) | 時系列を「トレンド + 季節性 + 残差」に分解し、残差が閾値を超えた点を異常と判定 |
...
[`series_decompose_anomalies`](https://learn.microsoft.com/ja-jp/azure/data-explorer/kusto/query/series-decompose-anomalies-function) は長期間のデータが必要なため：
...
let lookbackTime            = 1h;
// 同一 IP からの失敗回数閾値
let sprayFailureThreshold   = 20;
// 同一 IP が狙ったユニークアカウント数閾値
...
|-----|---------|-----------------|------|
| Lab 0 | RDP 接続検知 | Host エンティティで一致 | 同一端末への複数 RDP 接続をまとめる |
...
|---------|-----|-------------|-------------------|
| 特定条件 | Lab 0 / Lab 1 / Lab 4 | SecurityEvent / AzureActivity | `where EventID ==`、`where LogonType ==`、`where OperationNameValue =~`、`parse_json()` |
| 閾値 | Lab 2 / Lab 5 | SigninLogs / AzureActivity | `summarize count()`、`bin()`、`make_set()` |
| アノマリー | Lab 3 | SigninLogs | `make-series`、[`series_decompose_anomalies()`](https://learn.microsoft.com/ja-jp/azure/data-explorer/kusto/query/series-decompose-anomalies-function) |
